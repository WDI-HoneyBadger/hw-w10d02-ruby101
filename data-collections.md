...
```rb
ninja_turtles = ninja_turtles.merge({age: 209})
```
...
```rb
ninja_turtles = ninja_turtles.merge({pizza_toppings: ["cheese", "pepperoni", "peppers"]})
```
...
```rb
keys = []
ninja_turtles.each_key { |key| keys << key}
```
...
```rb
ninja_turtles.each_key do |key|
    puts "#{key} is equal to #{ninja_turtles[key]}"
end
```
